...
NaiveBayes <- R6Class("NaiveBayes",
                      public = list(
                        ...
                        # too long because we can't work with big dataset with Shiny
                        #check.numerical_parallel = function() {
                        #  # detect the number of cores available for parallelization
                        #  num_cores <- detectCores()
                        #  cl <- makeCluster(num_cores)
                        #  
                        #  # parLapply to check numerical columns in parallel
                        #  self$X <- parLapply(cl, self$X, function(column) {
                        #    if (is.numeric(column)) {
                        #      return(self$numerical.to.categorical(column))
                        #    } else {
                        #      return(column)
                        #    }
                        #  })
...
# TEST AVEC IRIS (A REGLER)
X <- iris[, c("Sepal.Length", "Sepal.Width", "Petal.Length", "Petal.Width")]
y <- iris[, "Species"]
X_test <- data.frame(
  Sepal.Length = c(5.3, 4.4, 5.7, 4.6, 5.2, 4.9),
  Sepal.Width = c(3.5, 3.4, 3.1, 3.1, 3.2, 3.0),
  Petal.Length = c(1.6, 1.4, 1.3, 1.5, 1.4, 1.7),
  Petal.Width = c(0.3, 0.2, 0.3, 0.2, 0.2, 0.4)
)
nb2 <- NaiveBayes$new(iris, c("Sepal.Length", "Sepal.Width", "Petal.Length", "Petal.Width"), "Species")
nb2$fit(X, y)

nb2$predict(X_test)
nb2$predict_proba(X_test)
nb2$conditional.probabilities
nb2$prior.probabilities


# TEST AVEC DONNEES COMPLETES
healthcare_data <- read.csv("healthcare_dataset.csv")
# Definir la seed pour la reproductibilite
set.seed(123)

library(caret)
# Spécifier les colonnes X et y
selected_columns <- c("Medical.Condition", "Admission.Type","Test.Results")

# Diviser les donnees en ensembles d'entrainement (70%) et de test (30%)
train_index <- createDataPartition(healthcare_data$Test.Results, p = 0.7, list = FALSE)
train_data <- healthcare_data[train_index,selected_columns ]
test_data <- healthcare_data[-train_index,selected_columns ]

X_train <- train_data[, -3]  # Remplacez colIndex par l'indice de la colonne cible
y_train <- train_data[, 3]   # Remplacez colIndex par l'indice de la colonne cible
X_test <- test_data[, -3]    # Remplacez colIndex par l'indice de la colonne cible
y_test <- test_data[, 3]     # Remplacez colIndex par l'indice de la colonne cible

nbh <- NaiveBayes$new(healthcare_data, c("Medical.Condition", "Admission.Type"), "Test.Results")
nbh$fit(X_train, y_train)
nbh$prior.probabilities
